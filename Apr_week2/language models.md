...
=====================================
## 트랜스포머 : BERT
- 마스크드 언어 모형 : 문장 내에서 가려진 부분에 나올 단어를 예측
- 두 문장을 입력 받았을 때 선후 관계인지 예측
- 전이학습에서 GPT보다 나은 성능, 자연어 생성은 불리 

=================
## 트랜스포터 기반 모형의 한계
1. gpt, bert 이외에도 다양한 모형이 개발
2. 트랜스포머 기반 모형의 한계는 계산량이 제곱으로 증가한다는 것
3. 병렬 처리가 가능하지만 문장이 너무 길어지면 한계
4. 긴 문장을 효율적으로 처리할 수 있는 모형에 대한 연구가 진행 

내일 제가 오전에 외출로 인해서 오후1시에 복귀합니다만 오늘처럼 프로젝트 시간이 주어진다면 오전에 주제에 대한 의견을 같이 못 나눌 것 같아서 지금 제 의견 보내드리는 게 좋을 거 같아서요!
  현재까지 생각해둔 것이랑 참고할 자료 같이 보내드려도 될까요? 보시고 내일 의견 나눌 때 조금이나마 도움이 되셨으면 합니다. . .
